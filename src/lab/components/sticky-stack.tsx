  ...
  return (
    <div className={cn("flex w-[min(480px,100%)] gap-2", className)}>
      <div
        ...
        aria-label={label}
        tabIndex={0}
        className="min-w-0 flex-1 overflow-y-auto overscroll-contain rounded-3xl bg-surface px-4 outline-none [scrollbar-width:none] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-foreground [&::-webkit-scrollbar]:hidden"
      ...
      <nav aria-label="Jump to card" className="flex flex-col justify-center">
        {items.map((item, i) => (
          <button
            ...
            onClick={() => goTo(i)}
            className="group flex h-10 w-6 touch-manipulation items-center justify-center rounded-full outline-none transition-[scale] duration-150 ease-out focus-visible:outline-2 focus-visible:outline-foreground active:scale-[0.96] motion-reduce:transition-none"
          >
